...
if ($_POST) {
    ...
    if (empty($errorMessage)) {
        // get_values.php Title and Blog Logo Control
        $seourl = "";
        if($GetBlogValues["title"] == $title){
            $seourl = $GetBlogValues["seourl"];
        }else{
            $seourl = convertURL($conn, $title);
        }
        if($GetBlogValues["main_image"] != $blog_logo){
            $blog_logo .= ".jpeg";
        }
        // end get_values.php Title and Blog Logo Control

        $date = date("Y-m-d H:i:s");
        $UpdateBlog = UpdateBlog($conn, /* get_values.php -> blog id */ $Blog_id, $blog_logo, $title, $content, $category, $seourl, $date, $post_now, $fixed);
        if ($UpdateBlog == "Success") {
            header("Location: show_blogs.php");
        }else {
            echo "<script>console.log('".$UpdateBlog."');</script>";
        }
    }else {
        $ErrorMessage_show = '
...
/* Functions */
// Save Blog
function UpdateBlog($connect, $blogID, $blog_logo, $title, $content, $category, $seourl, $date, $is_active, $fixed){
    $sql = "
    update blogs set 
    `main_image` = '$blog_logo', `title` = '$title', `content` = '$content', `category` = '$category', `seourl` = '$seourl', `date` = '$date', `is_active` = '$is_active', `fixed` = '$fixed'
    where id = ".(int)$blogID."
    ";
...
function sendMail($connect, $Blog_seourl, $msg_title, $msg_date){
    // Mail Infos
    ...
    while ($row = mysqli_fetch_array($query)) {
        ...
        $mail_content = HTMLDesignMail(
            ...
            "https://wp-1.ozceliksoftware.com?blog=$Blog_seourl", // -> Blog Link 
            "https://wp-1.ozceliksoftware.com/follow/functions/unfallow.php?email=".$row["email"]."" // -> Follower Unfallow Link
        );
        // Set Mail Message
        sendMail_smtp($row["email"], $mail_content, $row["email"], "Yeni Bir Blog Haberi");
    }
}
/* end Functions */
?>